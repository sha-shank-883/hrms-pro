...
async function resolveTenantByDevice(serialNumber) {
  ...
  if (result.rows.length === 0) {
    throw new NotFoundError(`Unregistered device: ${serialNumber}`);
  }

  const device = result.rows[0];
  if (device.status !== 'active') {
    throw new ValidationError(`Device ${serialNumber} is not active`);
  }
...
exports.handleUniversalWebhook = asyncHandler(async (req, res) => {
  const { deviceSerial, biometricId, timestamp, punchType } = req.body;

  if (!deviceSerial || !biometricId || !timestamp) {
    throw new ValidationError('Missing required fields: deviceSerial, biometricId, timestamp');
  }
...
exports.handleZktecoWebhook = asyncHandler(async (req, res) => {
  ...
  if (!snMatch) {
    ...
    if (!snParam) throw new ValidationError('No SN provided');
  }
