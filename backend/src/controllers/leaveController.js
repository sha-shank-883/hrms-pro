...
const getLeaveRequestById = asyncHandler(async (req, res) => {
  ...
  if (result.rows.length === 0) {
    throw new NotFoundError('Leave request not found or access denied');
  }
...
const createLeaveRequest = asyncHandler(async (req, res) => {
  ...
  if (daysUntilLeave < advanceNoticeDays) {
    throw new ValidationError(`Leave request must be submitted at least ${advanceNoticeDays} days in advance`);
  }
...
const updateLeaveRequest = asyncHandler(async (req, res) => {
  ...
  if (result.rows.length === 0) {
    throw new NotFoundError('Leave request not found or already processed');
  }
...
const getLeaveBalance = asyncHandler(async (req, res) => {
  ...
  if (userRole === 'employee') {
    ...
    if (employeeResult.rows.length === 0 || employeeResult.rows[0].employee_id != employee_id) {
      throw new ForbiddenError('Access denied. You can only view your own leave balance.');
    }
  }
...
const getAllLeaveBalances = asyncHandler(async (req, res) => {
  ...
  if (userRole !== 'admin' && userRole !== 'manager') {
    throw new ForbiddenError('Access denied. Only admins and managers can view all leave balances.');
  }
...
// Request Comp-off
const requestCompOff = asyncHandler(async (req, res) => {
  ...
  if (existing.rows.length > 0) {
    throw new ConflictError('A pending requests already exists for this date.');
  }
...
const getCompOffRequests = asyncHandler(async (req, res) => {
