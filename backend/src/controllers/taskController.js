...
const createTask = asyncHandler(async (req, res) => {
  ...
  if (title.length > 255) {
    throw new ValidationError('Task title must not exceed 255 characters');
  }
  if (priority && !['Low', 'Medium', 'High', 'Urgent', 'low', 'medium', 'high', 'urgent'].includes(priority.toString())) {
...
const getTaskUpdates = asyncHandler(async (req, res) => {
  const { task_id } = req.params;
...
const updateTaskUpdate = asyncHandler(async (req, res) => {
  ...
  if (ownerCheck.rows.length === 0) {
    throw new ForbiddenError('You can only edit your own updates');
  }
...
const deleteTaskUpdate = asyncHandler(async (req, res) => {
  const { update_id } = req.params;
  const user_id = req.user.userId;
