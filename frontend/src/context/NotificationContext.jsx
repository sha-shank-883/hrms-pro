...
export const NotificationProvider = ({ children }) => {
        ...
        try {
            ...
            if (res.success && res.counts) {
                setNotifications(prev => ({
                    ...prev,
                    ...res.counts,
                    total: (res.counts.leaves || 0) + (res.counts.attendance || 0) + (res.counts.tasks || 0) + (res.counts.chat || 0) + (res.counts.notifications || 0)
                }));
                setUnreadCount(res.counts.notifications || 0);
            }
        } catch (error) {
    ...
    const markAsRead = useCallback(async (idOrModule) => {
        if (typeof idOrModule === 'number' || (typeof idOrModule === 'string' && /^\d+$/.test(idOrModule))) {
            try {
                await notificationService.markAsRead(idOrModule);
                setFeed(prev => prev.map(item => item.id === Number(idOrModule) ? { ...item, is_read: true } : item));
                setUnreadCount(prev => Math.max(0, prev - 1));
                setNotifications(prev => ({ ...prev, notifications: Math.max(0, prev.notifications - 1) }));
            } catch (err) {
                console.error('Failed to mark notification as read:', err);
            }
        } else if (typeof idOrModule === 'string') {
            // Module level clear
            setNotifications(prev => ({ ...prev, [idOrModule]: 0 }));
        }
    }, []);
